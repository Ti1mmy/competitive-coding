...
for i in range(len(second)):
    if second[i][0] == "R":
        loc[x] += int(second[i][1:])
    elif second[i][0] == "L":
        loc[x] -= int(second[i][1:])
    elif second[i][0] == "U":
        loc[y] += int(second[i][1:])
    else:
        loc[y] -= int(second[i][1:])
    second_history += loc
    print(loc)
intersections = []
new_first = [[] for _ in range(len(first))]
new_second = [[] for _ in range(len(second))]
counter = 0
for i in range(len(first_history)):
    new_first[counter].append(first_history[i])
    if i % 2 and i > 0:
        counter += 1
counter = 0
for i in range(len(second_history)):
    new_second[counter].append(second_history[i])
    if i % 2 and i > 0:
        counter += 1
print(new_first)
print(new_second)
first_history = new_first
second_history = new_second
for i in range(len(first_history)):
...
"""
Ok so now that i have the points i need to draw lines between the points and find intersections
Maybe just flood in a bunch of points between each point for both arrays
"""